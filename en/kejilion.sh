...
SYSCTL
	...
	echo -e "${gl_lv}已应用 ${applied}Item parameter ${skipped:+, skip${skipped}Item unsupported parameter}${gl_bai}"
	...
	change_model() {
		...
		while true; do
			...
			if echo "$plugin_list" | grep -qw "$plugin_id" && echo "$plugin_list" | grep "$plugin_id" | grep -q "disabled"; then
				...
				openclaw plugins enable "$plugin_id" && echo "✅ 激活成功" || echo "❌ Activation failed"
...
EOF
		  ...
		  66)
			  ...
			  echo -e "6. 开放所有端口"
			  echo -e "7. Turn on${gl_huang}BBR${gl_bai}加速"
